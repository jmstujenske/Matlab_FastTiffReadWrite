...
%image_description (optional) = ImageDescription tiff tag as uint8
%
...
if predicted_filesize<3.9e9 %make sure less than 3.9GB, to be conservative
    ...
    TiffWriter=Fast_Tiff_Write(filename);
else
    disp('Big Video Detected. Saving as BigTiff.')
    TiffWriter=Fast_BigTiff_Write(filename);
end
tic;for a=frame_start:frame_end;TiffWriter.WriteIMG(vid(:,:,a)',imagedesc);end;close(TiffWriter);duration=toc;
